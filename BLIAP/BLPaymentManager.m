...
- (void)startTransactionObservingAndPaymentTransactionVerifingWithUserID:(NSString *)userid{
    ...
    // 检查沙盒中没有持久化的交易.
    [self checkUnfinishedTransactionInsSandbox];
}
...
- (void)buyProduct:(SKProduct *)product orderNo:(nonnull NSString *)orderNo error:(NSError *__autoreleasing  _Nullable * _Nullable)error {
    if ([self currentDeviceIsJailbroken]) {
        return;
    }
    NSParameterAssert(product);
    NSParameterAssert(orderNo.length);
    if (!product) {
    ...
    if (!orderNo.length) {
        NSError *e = [NSError errorWithDomain:BLWalletErrorDomain code:0 userInfo:@{NSLocalizedDescriptionKey: @"orderNo 为空"}];
        if (error) {
            *error = e;
        }
        return;
    ...
    SKMutablePayment *paymentM = [SKPayment paymentWithProduct:product].mutableCopy;
    paymentM.applicationUsername = orderNo;
    [[SKPaymentQueue defaultQueue] addPayment:paymentM.copy];
}
...
- (void)checkUnfinishedTransactionInsSandbox {
    // 未完成的列表.
...
- (BLPaymentTransactionModel *)generateTransactionModelWithPaymentTransaction:(SKPaymentTransaction *)transaction {
    return [[BLPaymentTransactionModel alloc]
            ...
            orderNo:transaction.payment.applicationUsername];
}
